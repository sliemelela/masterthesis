...
def myloss_alpha(pts, goal_pd, sliced=False, thetas=torch.tensor([k/4 * np.pi for k in range(5)])):
    ...
    diag = create_alpha_pd(pts)

    # Total persistence is a special case of Wasserstein distance
    if sliced == False:
        dist = wasserstein_distance(diag, goal_pd, order=1, enable_autodiff=True, keep_essential_parts=False)  
        return dist
    else:
        # dist = sliced_wasserstein_distance([diag, goal_pd], thetas[3])
        dists = [sliced_wasserstein_distance([diag, goal_pd], theta) for theta in thetas]
